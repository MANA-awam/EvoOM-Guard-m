...
| --- | --- | --- | --- |
| [`EvoOM-Guard-m`](https://github.com/EvoRiseKsa/EvoOM-Guard-m) | Authoritative source-available CLI, Action, releases, threat model, and security policy. | Source currently declares `3.8.0`. It becomes a product release only when the matching immutable GitHub Release is published; consumers must check [Releases](https://github.com/EvoRiseKsa/EvoOM-Guard-m/releases) before using a version pin. | External adoption, independent security review, or universal correctness. |
