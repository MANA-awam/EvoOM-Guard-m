...
> **Release availability.** The `v3.8.0` pins below become usable only after
> that exact GitHub Release is published. Before then, choose an existing
> published tag from the repository Releases page.
...
- uses: EvoRiseKsa/EvoOM-Guard-m@v3.8.0   # after its Release is published; @<sha> is strictest, @main is latest
```
...
```bash
pip install "git+https://github.com/EvoRiseKsa/EvoOM-Guard-m.git@v3.8.0"   # after its Release is published
...
> pip URL:
> - **`@v3.8.0`** — a release tag once published. The recommended pin and the right choice for
...
`EvoRiseKsa/EvoOM-Guard-m@v3.8.0` after its GitHub Release is published. Copy [`examples/evoguard.yml`](../examples/evoguard.yml) to
`.github/workflows/evoguard.yml` in the repo you want to protect:

```yaml
- uses: actions/checkout@9c091bb21b7c1c1d1991bb908d89e4e9dddfe3e0 # v7
  with: { fetch-depth: 0 }            # Guard needs the base commit to diff
- uses: EvoRiseKsa/EvoOM-Guard-m@v3.8.0   # after its Release is published (@<sha> strictest, @main latest)
  with:
...
- run: pip install "git+https://github.com/EvoRiseKsa/EvoOM-Guard-m.git@v3.8.0"   # after its Release is published; @<sha> strictest for CI
- run: |
